...
public class PageVO {
	...
	private boolean prev, next; // 이전, 다음페이지 유무

	private final int PAGE_BLOCK = 10; // 페이지블록 구성하는 최대 개수

	private int totalCount; // 전체 글 개수
	private CriteriaYP cri; // 요청 페이지 번호, 한 페이지 당 글 개수

	public PageVO(CriteriaYP cri, int totalCount) {
		this.cri = cri;
		...
		// 총 페이지가 32일 경우
		// totalCont(32) / cri.getAmount(30) 1.2 -> 2 => realEnd=2
	...
	public CriteriaYP getCri() {
		return cri;
	}

	public void setCri(CriteriaYP cri) {
		this.cri = cri;
	}
